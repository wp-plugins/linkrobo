...
class zipfile
{
      ...
      function create_zip($name) {
              $data = $this->zipped_file();
              if($f = fopen($name, "w")) {
                             fwrite($f, $data);
                             fclose;
                             }
      ...
      function write_info_in_zip($name, $file_data, $file_path) {

         if (!file_exists($name)) {
         	$this->create_file($file_data, $file_path);
                 $this->create_zip($name);
                 return;
                 }
         ...
         if ($fh = fopen($name, "a+")) {
                 ...
                 $prev_data = "";
                 $mode = "file data";
                 //$new_name = substr($name, 0, strrpos($name, ".")).time()."_tmp";
                 //copy($name, $new_name);
                         ...
                         fseek($fh, $size-$len);
                         $data = fread($fh, $len);
                         if (strstr($data, $this->eof_ctrl_dir)) {

                         $data = explode($this->eof_ctrl_dir, $data);
                         $data = $data[1];
                         $unpackeda = unpack("v1num_entries/v1num_entries_all/V1central_dir_size/V1central_dir_offset/v1comment_length", $data);

                         $position = $unpackeda['central_dir_offset'];
                         $central_dir_size = $unpackeda['central_dir_size'];

                         if ($central_dir_size<32*1024*1024) {
                                 fseek($fh, $position);
                                 $central_dir = fread($fh, $central_dir_size);

                 $this->old_offset = $unpackeda['central_dir_offset'];
                 $this->create_file($file_data, $file_path);

                 ftruncate($fh, $unpackeda['central_dir_offset']);
                 fwrite($fh, implode("", $this->datasec));
                 fwrite($fh, $central_dir.implode("", $this->ctrl_dir));

                                 $data = $this->eof_ctrl_dir.
                                         pack("v", $unpackeda['num_entries'] + sizeof($this->ctrl_dir)). // total number of entries "on this disk"
                                         pack("v", $unpackeda['num_entries'] + sizeof($this->ctrl_dir)). // total number of entries overall
                                         pack("V", $unpackeda['central_dir_size'] + strlen(implode("", $this->ctrl_dir))). // size of central dir
                                         pack("V", $unpackeda['central_dir_offset'] + strlen(implode("", $this->datasec))). // offset to start of central dir
                                         pack("v", $unpackeda['comment_length']); // .zip file comment length
                                         fwrite($fh, $data);
